    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p class="text-zinc-400 text-sm mt-1">Manage library book collections.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="p-5 font-bold">Details</th>
                            <th class="p-5 font-bold text-center">Stock</th>
                    ...
                    <tbody class="text-zinc-300 text-sm divide-y divide-zinc-800/50">
                        @forelse ($books as $book)
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="p-5 text-zinc-500 font-mono text-xs">#{{ $book->id }}</td>
                                <td class="p-5">
                                    <div class="flex flex-col">
                                        <span class="text-white font-bold text-base">{{ $book->nama }}</span>
                                        <span class="text-zinc-500 text-xs">{{ $book->pengarang }}</span>
                                    </div>
                                </td>
                                <td class="p-5">
                                    <div class="flex flex-col">
                                        <span class="text-zinc-300">{{ $book->penerbit }}</span>
                                        <span class="text-zinc-500 text-[11px]">{{ $book->tahun_terbit }}</span>
                                    </div>
                                </td>
                                <td class="p-5 text-center">
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border {{ $book->stock > 0 ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' : 'text-rose-400 bg-rose-400/10 border-rose-400/20' }}">
                                        {{ $book->stock }} units
                                    </span>
                                </td>
                                <td class="p-5">
